...
function WritableState:initialize(options, stream)
  ...
  --]]
    ...
    if core.instanceof(stream, require("stream/duplex").Duplex) then
        self.objectMode = self.objectMode or
                              not (not options.writableObjectMode)
    end
  ...
  --]]
    self.onwrite = function(er) onwrite(stream, er) end
...
function writeAfterEnd(stream, state, cb)
  ...
  --]]
    ...
    timer.setImmediate(function() cb(er) end)
end
...
function Writable:uncork()
    ...
    if state.corked ~= 0 then
        ...
        if not state.writing and state.corked == 0 and not state.finished and
            not state.bufferProcessing and table.getn(state.buffer) ~= 0 then
...
function writeOrBuffer(stream, state, chunk, cb)
  ...
  --]]
    if not ret then state.needDrain = true end
...
function onwrite(stream, er)
    ...
    --]]
        ...
        if not finished and state.corked == 0 and not state.bufferProcessing and
            table.getn(state.buffer) ~= 0 then clearBuffer(stream, state) end
...
function afterWrite(stream, state, finished, cb)
    if not finished then onwriteDrain(stream, state) end
    state.pendingcb = state.pendingcb - 1
    cb()
    ...
    --]]
      ...
      --]]
        ...
        if c <= table.getn(state.buffer) then
            ...
            for i = 1, c - 1 do table.remove(state.buffer, 1) end
        else
            state.buffer = {}
        end
    end
...
function Writable:_write(chunk, cb) cb(Error:new("not implemented")) end
...
function Writable:_end(chunk, cb)
    ...
    if chunk ~= nil then self:write(chunk) end
...
function needFinish(stream, state)
    return
        state.ending and state.length == 0 and table.getn(state.buffer) == 0 and
...
return {WriteReq = WriteReq, WritableState = WritableState, Writable = Writable}
