...
function ReadableState:initialize(options, stream)
  ...
  --]]
    ...
    local defaultHwm = 16
    if not options.objectMode then defaultHwm = 16 * 1024 end
  ...
  --]]
    ...
    if core.instanceof(stream, Stream.Duplex) then
        self.objectMode = self.objectMode or
                              (not (not options.readableObjectMode))
    end
...
function Readable:initialize(options)
    ...
    if type(Stream.initialize) == "function" then Stream.initialize(self) end
end
...
function readableAddChunk(stream, state, chunk, addToFront)
    ...
    elseif chunk == nil then
        state.reading = false
        if not state.ended then onEofChunk(stream, state) end
    elseif state.objectMode or chunk and len(chunk) > 0 then
        if state.ended and not addToFront then
            ...
            if not addToFront then state.reading = false end
      ...
      --]]
        ...
        --]]
                ...
                if state.needReadable then emitReadable(stream) end
            end
...
function needMoreData(state)
    return not state.ended and
               (state.needReadable or state.length < state.highWaterMark or
                   state.length == 0)
end
...
function howMuchToRead(n, state)
    if state.length == 0 and state.ended then return 0 end
...
function Readable:read(n)
    ...
    if type(n) ~= "number" or n > 0 then state.emittedReadable = false end
  ...
  --]]
    if n == 0 and state.needReadable and
        (state.length >= state.highWaterMark or state.ended) then
  ...
  --]]
    if n == 0 and state.ended then
        if state.length == 0 then endReadable(self) end
        return nil
    end
  ...
  --]]
    if state.ended or state.reading then doRead = false end
    ...
    --]]
        if state.length == 0 then state.needReadable = true end
        --[[
  ...
  --]]
    if doRead and not state.reading then n = howMuchToRead(nOrig, state) end
  ...
  --]]
    if state.length == 0 and not state.ended then state.needReadable = true end
  ...
  --]]
    ...
    if ret ~= nil then self:emit("data", ret) end
...
function emitReadable(stream)
    ...
    if not state.emittedReadable then
        ...
        if state.sync then
            timer.setImmediate(function() emitReadable_(stream) end)
        else
...
function maybeReadMore(stream, state)
    if not state.readingMore then
        ...
        timer.setImmediate(function() maybeReadMore_(stream, state) end)
    end
end
...
function Readable:pipe(dest, pipeOpts)
    ...
    onunpipe = function(readable) if readable == src then cleanup() end end

    onend = function() dest:_end() end
  ...
  --]]
    ...
    unpipe = function() src:unpipe(dest) end
  ...
  --]]
    if not state.flowing then src:resume() end
...
function pipeOnDrain(src)
    return function()
        ...
        if state.awaitDrain == 0 and core.Emitter.listenerCount(src, "data") ~=
            0 then
            state.flowing = true
            flow(src)
        end
...
function Readable:unpipe(dest)
  ...
  --]]
    if state.pipesCount == 0 then return self end
  ...
  --]]
    ...
    --]]
        if dest and dest ~= state.pipes then return self end

        if not dest then dest = state.pipes end
    ...
    --]]
        ...
        state.pipesCount = 0
        state.flowing = false
        if dest then dest:emit("unpipe", self) end
        return self
    end
  ...
  --]]
    ...
    --]]
        ...
        for i = 1, l, 1 do dests[i]:emit("unpipe", self) end
        return self
    end
  ...
  --]]
    local i
    for j, pipe in ipairs(state.pipes) do if pipe == dest then i = j end end
    if i == nil then return self end

    table.remove(state.pipes, i)
    state.pipesCount = state.pipesCount - 1
    if state.pipesCount == 1 then state.pipes = state.pipes[1] end
    ...
    if ev == "readable" and self.readable then
        ...
        if not state.readableListening then
            ...
            if not state.reading then
                local _self = self
                timer.setImmediate(function() _self:read(0) end)
...
function Readable:resume()
    ...
    if not state.flowing then
        state.flowing = true
        if not state.reading then self:read(0) end
        resume(self, state)
    end
    return self
end
...
function resume(stream, state)
    if not state.resumeScheduled then
        ...
        timer.setImmediate(function() resume_(stream, state) end)
    end
end
...
function resume_(stream, state)
    ...
    stream:emit("resume")
    flow(stream)
    if state.flowing and not state.reading then stream:read(0) end
end
...
function flow(stream)
    ...
    if state.flowing then
        ...
        while nil ~= chunk and state.flowing do chunk = stream:read() end
    end
end
...
function Readable:wrap(stream)
  ...
  --]]
    ...
    for _, v in pairs(events) do stream:on(v, utils.bind(self.emit, self, v)) end
...
function endReadable(stream)
  ...
  --]]
    if state.length > 0 then error("endReadable called on non-empty stream") end
...
return {Readable = Readable, ReadableState = ReadableState}
