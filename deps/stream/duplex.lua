...
for k, v in pairs(Writable) do
    if not Duplex[k] and k ~= "meta" then Duplex[k] = v end
end
...
function Duplex:initialize(options)
  ...
  --]]
    ...
    if options and options.readable == false then self.readable = false end

    if options and options.writable == false then self.writable = false end
...
function onend(self)
  ...
  // then we're ok.
  --]]
    if self.allowHalfOpen or self._writableState.ended then return end
