...
function Path:_get(key) return self[key] end

function Path:getRoot(filepath) return self.root end

function Path:getSep() return self.sep end

function Path:pathsEqual(a, b) return a == b end
...
function Path:normalize(filepath)
    ...
    if root then filepath = filepath:sub(root:len() + 1) end
    ...
    if #filepath == 0 then
        if is_absolute then return root end
        return "."
    end
    if trailing_slash then filepath = filepath .. self.sep end
    if is_absolute then filepath = root .. filepath end
    return filepath
end
...
function Path:_filterparts(parts)
    ...
    for i, part in ipairs(parts) do
        if part and part ~= "" then table.insert(filteredparts, part) end
    end
    for i, part in ipairs(filteredparts) do
        -- Strip leading slashes on all but first item
        if i > 1 then
            while part:sub(1, 1) == self.sep do part = part:sub(2) end
        end
...
function Path:_rawjoin(parts) return table.concat(parts, self.sep) end

function Path:_filteredjoin(...)
    local parts = {...}
    for i, part in ipairs(parts) do parts[i] = self:normalizeSeparators(part) end
...
function Path:relative(from, to)
    ...
    if not self:pathsEqual(from_root, to_root) then return to end
...
function PosixPath:initialize() Path.initialize(self, '/', '/') end

function PosixPath:isAbsolute(filepath)
    return filepath:sub(1, self.root:len()) == self.root
end

function PosixPath:isUNC(filepath) return false end

function PosixPath:isDriveRelative(filepath) return false end

function PosixPath:normalizeSeparators(filepath) return filepath end

function PosixPath:_makeLong(filepath) return filepath end

local WindowsPath = Path:extend()

function WindowsPath:initialize() Path.initialize(self, 'c:\\', '\\') end

-- Windows paths are case-insensitive
function WindowsPath:pathsEqual(a, b) return a and b and a:lower() == b:lower() end

function WindowsPath:isAbsolute(filepath)
    return filepath and not self:isDriveRelative(filepath) and
               self:getRoot(filepath) ~= nil
end
    ...
    if filepath then
        if self:isUNC(filepath) then
            ...
            local share = filepath:sub(server:len() + 3):match(
                              "^[\\/]([^\\/.][^\\/]*)")
            local root = self.sep .. self.sep .. server ..
                             (share and (self.sep .. share) or "")
            ...
            local sep = (drive and not self:isDriveRelative(filepath)) and
                            self.sep or ""
...
return {nt = WindowsPath:new(), posix = PosixPath:new()}
