...
function fs.readFile(path)
    ...
    if stat then
        ...
        if stat.size == 0 and stat.birthtime.sec == 0 and stat.birthtime.nsec ==
            0 then
            ...
            if not err then data = table.concat(buffs) end
        else
...
function fs.chroot(base)
    ...
    function chroot.mkdir(path, mode) return fs.mkdir(resolve(path), mode) end
    function chroot.mkdirp(path, mode) return fs.mkdirp(resolve(path), mode) end
    function chroot.open(path, flags, mode)
        return fs.open(resolve(path), flags, mode)
    end
    function chroot.unlink(path) return fs.unlink(resolve(path)) end
    function chroot.stat(path) return fs.stat(resolve(path)) end
    function chroot.lstat(path) return fs.lstat(resolve(path)) end
    ...
    function chroot.readlink(path) return fs.readlink(resolve(path)) end
    function chroot.chmod(path, mode) return fs.chmod(resolve(path), mode) end
    ...
    function chroot.rmdir(path) return fs.rmdir(resolve(path)) end
    function chroot.rmrf(path) return fs.rmrf(resolve(path)) end
    function chroot.scandir(path, iter)
        return fs.scandir(resolve(path), iter)
    end
    function chroot.readFile(path) return fs.readFile(resolve(path)) end
