...
local DEFAULT_SERVERS = {
    {['host'] = '8.8.8.8', ['port'] = 53, ['tcp'] = false},
    {['host'] = '8.8.4.4', ['port'] = 53, ['tcp'] = false}
}
...
local function _encode_name(s) return char(#s) .. s end
...
local function _decode_name(buf, pos)
    ...
    while nptrs < 128 do
        ...
        if not fst then return nil, 'truncated'; end

        -- print("fst at ", p, ": ", fst)

        if fst == 0 then
            if nptrs == 0 then pos = pos + 1 end
            break
        end

        if band(fst, 0xc0) ~= 0 then
            -- being a pointer
            if nptrs == 0 then pos = pos + 2 end

            nptrs = nptrs + 1

            local snd = byte(buf, p + 1)
            if not snd then return nil, 'truncated' end
...
local function _build_request(qname, id, no_recurse, opts)
    ...
    if opts then qtype = opts.qtype end
    ...
    if byte(qname, 1) == DOT_CHAR then return nil, "bad name" end
...
local function parse_response(buf, id, server)
    local n = #buf
    if n < 12 then return nil, 'truncated'; end
    ...
    if band(flags, 0x200) ~= 0 then return nil, "truncated" end
    ...
    if not ans_qname then return nil, pos end
    ...
    for i = 1, nan do
        ...
        elseif typ == TYPE_CNAME then
            ...
            if not cname then return nil, pos end

            if p - pos ~= len then
                return nil, format("bad cname record length: %d ~= %d", p - pos,
                                   len)
            end
        ...
        elseif typ == TYPE_MX then
            ...
            if not host then return nil, pos end
        ...
        elseif typ == TYPE_SRV then
            ...
            if not recname then return nil, pos end

            if p - pos ~= len then
                return nil,
                       format("bad srv record length: %d ~= %d", p - pos, len)
            end
...
local function setServers(servers) SERVERS = servers end

local function setTimeout(timeout) TIMEOUT = timeout end

local function setDefaultTimeout() TIMEOUT = DEFAULT_TIMEOUT end

local function setDefaultServers() SERVERS = DEFAULT_SERVERS end
...
local function loadResolverWin(options)
    ...
    if rv == ffi.C.ERROR_SUCCESS then
        ...
        repeat
            ...
            nextp = addr.Next
            if nextp ~= nil then addr = nextp[0] end
        until nextp == nil
    end

    if #servers then SERVERS = servers end
    return servers
end
...
local function loadResolverUnix(options)
    ...
    options = options or {file = '/etc/resolv.conf'}
