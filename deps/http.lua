...
function ServerResponse:flushHeaders()
    ...
    local head = {}
    local sent_date, sent_connection, sent_transfer_encoding,
          sent_content_length
    for i = 1, #headers do
...
function ClientRequest.getDefaultUserAgent()
    if ClientRequest._defaultUserAgent == nil then
        ClientRequest._defaultUserAgent = 'rocks/http'
    end
                ...
                if R == true then
                    ...
                    if type(event) == "table" then
                        ...
                        if self.method == 'CONNECT' or res.headers.upgrade then
                            local evt =
                                self.method == 'CONNECT' and 'connect' or
                                    'upgrade'
                            ...
                            elseif self.method == 'CONNECT' or res.statusCode ==
                                101 then
                                return self:destroy()
                            end
                        end
...
function ClientRequest:_done(data, cb)
    self:_end(data, function() if cb then cb() end end)
end

function ClientRequest:_setConnection()
    if not self.connection then table.insert(self, {'connection', 'close'}) end
end
...
function ClientRequest:setTimeout(msecs, callback)
    if self.socket then self.socket:setTimeout(msecs, callback) end
end
