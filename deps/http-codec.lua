...
local function encoder()
    ...
    function encodeHead(item)
        ...
        elseif not (type(item) == "table") then
            error("expected a table but got a " .. type(item) ..
                      " when encoding data")
        end
        ...
        if item.method then
            ...
            head = {
                item.method .. ' ' .. item.path .. ' HTTP/' .. version .. '\r\n'
            }
        else
            local reason = item.reason or STATUS_CODES[item.code]
            head = {
                'HTTP/' .. version .. ' ' .. item.code .. ' ' .. reason ..
                    '\r\n'
            }
        end
        for i = 1, #item do
...
local function decoder()
    ...
    function decodeHead(chunk, index)
        ...
        if head.keepAlive and
            (not (chunkedEncoding or (contentLength and contentLength > 0))) or
    ...
    mode = decodeHead
    return function(chunk, index) return mode(chunk, index) end
