...
function Timer:_update() self._active = uv.is_active(self._handle) end
...
local function peek(list)
    if list._idlePrev == list then return nil end
    return list._idlePrev
end

local function remove(item)
    if item._idleNext then item._idleNext._idlePrev = item._idlePrev end

    if item._idlePrev then item._idlePrev._idleNext = item._idleNext end
...
local function isEmpty(list) return list._idleNext == list end
    ...
    return function()
        ...
        while peek(timer) do
            ...
            if ((diff + 1) < msecs) == true then
                ...
                remove(elem)
                if elem.emit then elem:emit("timeout") end
            end
        end
...
local function enroll(item, msecs)
    if item._idleNext then unenroll(item) end
    item._idleTimeout = msecs
    init(item)
end
