...
local Cursor = require("mongodb/cursor")
...
function Collection:find(query, cb) return Cursor:new(self, query, cb) end
...
function Collection:distinct(field, query, cb)
    ...
    local cmd = {
        {"_order_", "distinct", self.collectionName}, {"_order_", "key", field}
    }
    if query then table.insert(cmd, {"_order_", "query", query}) end
...
function Collection:ensureIndex(index, cb) self:createIndex(index, cb) end
    ...
    if type(index) == "string" then
        ...
        if #index > 0 then
            for k, v in pairs(index) do
                if type(k) == "number" then k = v[2] end
                name = name .. "_" .. k
            end
        else
            for k, v in pairs(index) do name = name .. "_" .. k end
        end
        local indexElem = {key = index, name = name}
        for k, v in pairs(options) do indexElem[k] = v end
        indexes = {indexElem}
    end
...
function Collection:getIndex(cb)
    local collectionName = self.collectionName
    local indexes = "*"
    local cmd = {["listIndexes"] = collectionName}
    self.db:query("$cmd", cmd, nil, nil, 1, function(err, res)
        local result = res[1]
        if result.ok == 1 then
            cb(err, result)
...
function Collection:findOne(query, cb)
    ...
    return Cursor:new(self, query):limit(1):exec(
               function(err, res)
            if err then
                cb(err)
                return false
...
function Collection:renameCollection(newName, cb)
    ...
    self.db:query("$cmd", cmd, nil, nil, 1, function(err, res) cb(err, res) end,
                  nil)
end
