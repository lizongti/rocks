...
function Cursor:skip(skip, cb)
    if skip then self._skip = skip end
    if cb and type(cb) == "function" then
        self.cb = cb
        self:_exec()
    end
...
function Cursor:limit(limit, cb)
    if limit then self._limit = limit end
    if cb and type(cb) == "function" then
        self.cb = cb
        self:_exec()
    end
...
function Cursor:sort(doc, cb)
    if doc then self._sort = doc end
    if cb and type(cb) == "function" then
        self.cb = cb
        self:_exec()
    end
...
function Cursor:update(update, cb)
    self.action = "UPDATE"
    if update then self._update = update end
    if cb and type(cb) == "function" then
        self.cb = cb
        self:_exec()
    end
...
function Cursor:_exec()
    if self.action == "COUNT" then
        local cmd = {
            {"_order_", "count", self.collectionName},
            {"_order_", "query", self.query}
        }
    ...
    if self._sort then
        if self.query and not self.query["$query"] or not self.query then
            self.query = {
                {"_order_", "$query", self.query},
                {"_order_", "$orderby", self._sort}
            }
        end
    end
    self.db:find(self.collectionName, self.query, self._fields, self._skip,
                 self._limit, function(err, res)
        if self._update and self.action == "UPDATE" then
            if next(res) == nil then
                ...
                self.db:update(self.collectionName, {["$or"] = ids},
                               self._update, nil, 1, self.cb)
            end
        elseif self.action == "REMOVE" then
            if next(res) == nil then
                ...
                self.db:remove(self.collectionName, {["$or"] = ids}, nil,
                               self.cb)
            end
        else
            self.cb(err, res)
