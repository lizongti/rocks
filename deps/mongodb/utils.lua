...
local le_uint_to_num = function(s, i, j)
    ...
    local n = 0
    for i = #b, 1, -1 do n = n * 2 ^ 8 + b[i] end
    return n
end
local le_int_to_num = function(s, i, j)
    ...
    if n > overflow then n = (n % overflow) - overflow end
    return n
end
local num_to_le_uint = function(n, bytes)
    bytes = bytes or 4
    local b = {}
    for i = 1, bytes do b[i], n = n % 2 ^ 8, floor(n / 2 ^ 8) end
...
local be_uint_to_num = function(s, i, j)
    ...
    local b = {strbyte(s, i, j)}
    local n = 0
    for i = 1, #b do n = n * 2 ^ 8 + b[i] end
    return n
end
local num_to_be_uint = function(n, bytes)
    bytes = bytes or 4
    local b = {}
    for i = bytes, 1, -1 do b[i], n = n % 2 ^ 8, floor(n / 2 ^ 8) end
    assert(n == 0)
    return strchar(unpack(b))
end
