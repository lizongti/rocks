    ...
    for i = 1, #str do
        repeat
            ...
            if append ~= 0 then
                ...
                append = append - 1
                if append == 0 then index = index + 1 end
                break
            end

            local charLen = chlen(byte)
            if charLen == 0 and not allowInvaild then
                error("Invaild UTF-8 sequence at " .. tostring(i) .. ",byte:" ..
                          tostring(byte))
            end
            ustr[index] = char
...
function ustring.copy(ustr)
    local u = ustring.new()
    for i = 1, #ustr do u[i] = ustr[i] end
    return u
end
...
function ustring.uindex2index(ustr, uindex, initrawindex, inituindex)
    ...
    if uindex < 0 then
        local index = 0
        for i = ulen, ulen + uindex + 1, -1 do index = index + #ustr[i] end
        return -index
    else
        local index = (inituindex or 1)
        inituindex = inituindex or 1
        for i = inituindex, uindex - 1 do index = index + #ustr[i] end
        return index
    end
end
...
function ustring.gsub(ustr, pattern, repl, n)
    return ustring.new(
               gsub(tostring(ustr), tostring(pattern), tostring(repl), n))
end
...
function ustring.sub(ustr, i, j)
    ...
    for ii = i, math.min(j, len) do u[#u + 1] = ustr[ii] end
    return u
end

function ustring.find(ustr, pattern, init, plain)
    local first, last = find(tostring(ustr), tostring(pattern),
                             ustring.uindex2index(ustr, init), plain)
...
function ustring.gmatch(ustr, pattern) return gmatch(tostring(ustr), pattern) end

function ustring.match(ustr, pattern, init)
    return match(tostring(ustr), tostring(pattern),
                 ustring.uindex2index(ustr, init))
end

function ustring.lower(ustr)
    local u = ustring.copy(ustr)
    for i = 1, #u do u[i] = lower(u[i]) end
    return u
end

function ustring.upper(ustr)
    local u = ustring.copy(ustr)
    for i = 1, #u do u[i] = upper(u[i]) end
    return u
end

function ustring.rep(ustr, n)
    local u = ustring.new()
    for i = 1, n do for ii = 1, #ustr do u[#u + 1] = ustr[ii] end end
    return u
end
...
function ustring.reverse(ustr)
    local u = ustring.copy(ustr)
    local len = #ustr;
    for i = 1, len do u[i] = ustr[len - i + 1] end
    return u
end
...
function _meta.__eq(ustr1, ustr2)
    ...
    for i = 1, len1 do if ustr1[i] ~= ustr2[i] then return false end end
    return true
end

function _meta.__tostring(self) return tostring(table.concat(self)) end

function _meta.__concat(ustr1, ustr2)
    local u = ustring.copy(ustr1)
    for i = 1, #ustr2 do u[#u + 1] = ustr2[i] end
    return u
end
