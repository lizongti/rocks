...
function Process:setHandle(handle) self.handle = handle end

function Process:setPid(pid) self.pid = pid end
...
function Process:destroy(err)
    if err then timer.setImmediate(function() self:emit('error', err) end) end
end
...
local function spawn(command, args, options)
    ...
    local function maybeClose()
        ...
        if closesGot == 0 then em:emit('close', em.exitCode, em.signal) end
    end
...
local function _exec(file, args, options, callback)
    local opts = {timeout = 0, maxBuffer = 4 * 1024, signal = 'SIGTERM'}
    table.foreach(opts,
                  function(k, v) if not options[k] then options[k] = v end end)
    ...
    local function onClose(_exitCode) exitHandler(_exitCode, nil) end
    local function kill()
...
local function exec(command, options, callback)
    if not callback and
        (type(options) == "thread" or type(options) == "function") then
        callback = options
        options = {}
    end
...
return {exec = exec, execFile = execFile, spawn = spawn}
