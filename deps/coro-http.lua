...
local function parseUrl(url)
    local protocol, host, hostname, port, path =
        url:match("^(https?:)//(([^/:]+):?([0-9]*))(/?.*)$")
...
local function getConnection(host, port, tls, timeout)
    for i = #connections, 1, -1 do
        ...
        if connection.host == host and connection.port == port and
            connection.tls == tls then
...
local function request(method, url, headers, body, timeout)
    ...
    local req = {method = method, path = uri.path, {"Host", uri.host}}
    local contentLength
    local chunked
    if headers then
        for i = 1, #headers do
