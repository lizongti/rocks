local function makeCloser(socket)
    local closer = {read = false, written = false, errored = false}
    ...
    local function close()
        ...
        if not closer.readClosed then
            ...
            if closer.onClose then closer.onClose() end
        end
...
local function makeWrite(socket, closer)
    ...
    local function wait()
        ...
        return function(err) assert(coroutine.resume(thread, err)) end
    end

    local function write(chunk)
        if closer.written then return nil, "already shutdown" end
...
local function wrapStream(socket)
    assert(
        socket and socket.write and socket.shutdown and socket.read_start and
            socket.read_stop and socket.is_closing and socket.close,
        "socket does not appear to be a socket/uv_stream_t")

    local closer = makeCloser(socket)
    return makeRead(socket, closer), makeWrite(socket, closer), closer.close
end

return {wrapRead = wrapRead, wrapWrite = wrapWrite, wrapStream = wrapStream}
