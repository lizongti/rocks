...
local function formatError(msg)
    return string.format("%s: %s %s:%s (%s) - %s", msg.S or "?", msg.F or "?",
                         msg.L or "?", msg.C or "?", msg.R or "?", msg.M or "?")
end
...
local function wrap(options, read, write, socket)
    ...
    write {
        'StartupMessage', {user = options.username, database = options.database}
    }
        ...
        for message in read do
            if message[1] == "ErrorResponse" then
                if waiting then
                    ...
                    return assert(coroutine.resume(t, nil,
                                                   formatError(message[2]),
                                                   message[2]))
                end
                error(formatError(message[2]))
    ...
    local function execute(name, ...)
        write {
            {'Bind', '', name, ...}, {'Describe', 'P', ''}, {'Execute', '', 0},
            {'Sync'}
        }
        waiting = coroutine.running()
        return coroutine.yield()
    ...
    local function close() return write() end
...
return {connect = connect, wrap = wrap}
