...
local function writeCstring(string) return string .. "\0" end
...
local function writeUint32(int)
    return char(band(rshift(int, 24), 0xff), band(rshift(int, 16), 0xff),
                band(rshift(int, 8), 0xff), band(int, 0xff))
end

local function writeUint32List(list)
    local parts = {}
    for i = 1, #list do parts[i] = writeUint32(list[i]) end
    return concat(parts)
end
...
local function readCString(string, index)
    ...
    while byte(string, index) > 0 do index = index + 1 end
...
local function readCStringList(string, index)
    if byte(string, index) == 0 then return {}, index + 1 end
    local list = {}
    local i = 1
...
local function readUint16(string, index)
    return bor(lshift(byte(string, index), 8), byte(string, index + 1)),
           index + 2
end
...
local function readInt32(string, index)
    return bor(lshift(byte(string, index), 24),
               lshift(byte(string, index + 1), 16),
               lshift(byte(string, index + 2), 8), byte(string, index + 3)),
           index + 4
end
...
function encoders.Describe(name, type)
    return frame('D', type .. writeCstring(name))
end

function encoders.Execute(name, max_rows)
    return frame('E', writeCstring(name) .. writeUint32(max_rows))
end

function encoders.Flush() return frame('H', "") end
...
function encoders.Parse(name, query, var_types)
    ...
    return frame('P',
                 concat(writeCstring(name), writeCstring(query),
                        writeUint16(#var_types), writeUint32List(var_types)))
end
...
function encoders.Bind(portal_name, source_name, ...)
    ...
    local values = {...}
    local message = {
        writeCstring(portal_name), writeCstring(source_name), writeUint16(0), -- all parameters are text
        writeUint16(nargs)
    }
    for i = 1, nargs do
        local val = values[i]
...
function encoders.PasswordMessage(password)
    ...
    return frame('p', writeCstring(password))
end

function encoders.Query(query) return frame('Q', writeCstring(query)) end

function encoders.SSLRequest() return frame('', writeUint32(80877103)) end

function encoders.StartupMessage(options)
    -- Protocol version number 3
    return frame('', writeUint32(196608) .. writeHash(options))
end

function encoders.Sync() return frame('S', '') end

function encoders.Terminate() return frame('X', '') end
...
local function encode(message)
    ...
    if type(request) == 'table' then
        local parts = {}
        for i = 1, #message do parts[i] = encode(message[i]) end
        return concat(parts)
    end

    local formatter = encoders[request]

    if not formatter then error('No such request type: ' .. request) end
...
-- Output is (type, data...)
local parsers = {
    [byte('2', 1)] = function(string) return 'BindCompletion' end,
    ...
    [byte('R', 1)] = function(string)
        ...
        if not authType then error('Unknown auth code: ' .. authCode) end
    ...
    [byte('E', 1)] = function(string)
        ...
        local map = {}
        for i = 1, #list do map[sub(list[i], 1, 1)] = sub(list[i], 2) end
...
return {decode = decode, encode = encode, encoders = encoders}
