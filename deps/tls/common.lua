...
local isTLSv1_3 = function()
    if isLibreSSL() then return false end
...
local getSecureOptions = function(protocol, flags)
    return bit.bor(openssl.ssl.no_sslv2, openssl.ssl.no_sslv3,
                   openssl.ssl.no_compression, flags or 0)
end
...
do
    ...
    local len = #data
    while index < len do
        local len = bit.bor(bit.lshift(data:byte(index), 8),
                            data:byte(index + 1))
        index = index + 2
...
function Credential:initialize(secureProtocol, defaultCiphers, flags,
    ...
    if context then
        ...
        self.context = openssl.ssl.ctx_new(
                           secureProtocol or DEFAULT_SECUREPROTOCOL,
...
function Credential:addRootCerts() self.context:cert_store(DEFAULT_CA_STORE) end
...
function TLSSocket:initialize(socket, options)
    ...
    if socket then self._connecting = socket._connecting end

    self:once('end', function() self:destroy() end)

    self:read(0)
end

function TLSSocket:_init()
    self.ctx = self.options.secureContext or self.options.credentials or
    ...
    if (not self.server) then
        ...
        if self.ctx.session then self.ssl:session(self.ctx.session) end
    end
end

function TLSSocket:version() return self.ssl:get('version') end

function TLSSocket:getPeerCertificate() return self.ssl:peer() end
...
function TLSSocket:destroy(err)
    ...
    local function shutdown()
        ...
        if self._shutdown then
            ...
            if (shutdown_err == "want_read" or shutdown_err == "want_write" or
                shutdown_err == "syscall") then
    ...
    local function onShutdown(read_err, data)
        timer.active(self)
        if read_err or not data then return reallyShutdown() end
        timer.active(self._shutdownTimer)
        self.inp:write(data)
        shutdown()
    end
...
function TLSSocket:_write(data, callback)
    ...
    if data then
        ...
        if ret == nil then return self:destroy(err) end
    end
    self:flush(callback)
end
...
function TLSSocket:_read(n)
    ...
    function handshake()
        ...
        if self.ssl:peek() then incoming() end
...
function createCredentials(options, context)
    ...
    if context then return ctx end
