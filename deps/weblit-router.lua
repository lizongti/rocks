...
local function compileGlob(glob)
    ...
    for a, b in glob:gmatch("([^*]*)(%**)") do
        if #a > 0 then parts[#parts + 1] = escape(a) end
        if #b > 0 then parts[#parts + 1] = "(.*)" end
    end
    parts[#parts + 1] = "$"
    local pattern = table.concat(parts)
    return function(string) return string and string:match(pattern) end
end
...
local function compileRoute(route)
    ...
    for a, b, c, d in route:gmatch("([^:]*):([_%a][_%w]*)(:?)([^:]*)") do
        ...
        names[#names + 1] = b
        if #d > 0 then parts[#parts + 1] = escape(d) end
    end
    if #parts == 1 then
        return function(string) if string == route then return {} end end
    end
    parts[#parts + 1] = "$"
    local pattern = table.concat(parts)
...
local function newRouter()
    ...
    function router.route(options, handler)
        ...
        router.use(function(req, res, go)
            ...
            if path then
                ...
                if #query > 0 then req.query = parseQuery(query) end
            end
    ...
    function router.run(req, res, go)
        ...
        local function run(i)
            if i > len then return (go or function() end)() end
            return handlers[i](req, res, function() return run(i + 1) end)
        end
        run(1)
    end
