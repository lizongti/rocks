...
local function bin(str) return buf(#str, str) end

local tags = {}
local function registerTag(num, tagDecoder) tags[num] = tagDecoder end

local encoders = {}
local function encode(obj) return encoders[type(obj)](obj) end

local function encode_integer(major, num)
    assert(num >= 0)
    if num >= 0x100000000 and type(num) == 'number' then num = u64(num) end
    if istype(u64, num) then
        return concat {
            char(bor(major, 27)), char(tonumber(rshift(num, 56))),
    ...
    elseif num < 0x10000 then
        return concat {
            char(bor(major, 25)), char(rshift(num, 8)), char(band(num, 0xff))
        }
    else
        return concat {
            char(bor(major, 26)), char(rshift(num, 24)),
            char(band(rshift(num, 16), 0xff)), char(band(rshift(num, 8), 0xff)),
            char(band(num, 0xff))
        }
    end
end

encoders['nil'] = function() return '\xf6' end

encoders.boolean = function(bool) return bool and '\xf5' or '\xf4' end
...
encoders.string = function(str) return encode_integer(0x60, #str) .. str end

encoders.cdata = function(val)
    if istype(u64, val) then return encode_integer(0x00, val) end
...
encoders.table = function(tab)
    ...
    if is_array then
        ...
        for i = 1, len do parts[i + 1] = encode(tab[i]) end
        return concat(parts)
    else
        local parts = {}
        local count = 0
...
local function decode_u16(chunk, index)
    return bor(lshift(byte(chunk, index), 8), byte(chunk, index + 1)), index + 2
end

local function decode_u32(chunk, index)
    return bor(lshift(byte(chunk, index), 24),
               lshift(byte(chunk, index + 1), 16),
               lshift(byte(chunk, index + 2), 8), byte(chunk, index + 3)),
           index + 4
end

local function decode_u64(chunk, index)
    return bor(lshift(u64(byte(chunk, index)), 56),
               ...
               u64(byte(chunk, index + 7))), index + 8
end
