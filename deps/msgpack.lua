...
local function write16(num) return char(rshift(num, 8), band(num, 0xff)) end

local function write32(num)
    return char(rshift(num, 24), band(rshift(num, 16), 0xff),
                band(rshift(num, 8), 0xff), band(num, 0xff))
end
...
local function read32(data, offset)
    return bor(lshift(byte(data, offset), 24),
               lshift(byte(data, offset + 1), 16),
               lshift(byte(data, offset + 2), 8), byte(data, offset + 3))
end
...
local function encode(value)
    ...
    elseif t == "number" then
        ...
        elseif floor(value) ~= value then
            ...
            if bigEndian then
                return char(0xCB, bytes[0], bytes[1], bytes[2], bytes[3],
                            bytes[4], bytes[5], bytes[6], bytes[7])
            else
                return char(0xCB, bytes[7], bytes[6], bytes[5], bytes[4],
            ...
            if value >= 0 then
                ...
                elseif value < 0x100000000 then
                    ...
                    return "\xcf" .. write32(floor(value / 0x100000000)) ..
    ...
    elseif t == "table" then
        ...
        if isMap then
            ...
            local parts = {}
            local l = max
            for i = 1, l do parts[i] = encode(value[i]) end
            value = concat(parts)
            if l < 0x10 then
...
local function decode(data, offset)
    ...
    elseif c == 0xca then
        if bigEndian then
            ...
            copy(fbox,
                 char(byte(data, offset + 5), byte(data, offset + 4),
                      byte(data, offset + 3), byte(data, offset + 2)))
        end
        return fbox[0], 5
    elseif c == 0xcb then
        if bigEndian then
            ...
            copy(dbox,
                 char(byte(data, offset + 9), byte(data, offset + 8),
                      byte(data, offset + 7), byte(data, offset + 6),
                      byte(data, offset + 5), byte(data, offset + 4),
                      byte(data, offset + 3), byte(data, offset + 2)))
        end
        return dbox[0], 9
    elseif c == 0xdc then
    ...
    elseif c == 0xdd then
        return readarray(read32(data, offset + 2) % 0x100000000, data, offset,
                         offset + 5)
    elseif c == 0xde then
        return readmap(read16(data, offset + 2), data, offset, offset + 3)
    elseif c == 0xdf then
        return readmap(read32(data, offset + 2) % 0x100000000, data, offset,
                       offset + 5)
    else
...
return {
    encode = encode,
    decode = function(data, offset) return decode(data, offset or 0) end
}
