...
local function decoder(read, decode)
    ...
    return function()
        ...
        while true do
            ...
            if buffer == '' or buffer == nil then return nil end
            ...
            if item or newIndex then
                ...
                if newIndex then
                    assert(type(newIndex) == "number",
                           "index must be a number if set")
    ...
    end, function(newDecode) decode = newDecode end
end

local function encoder(write, encode)
    return function(item)
        if not item then return write() end
        return write(encode(item))
    end, function(newEncode) encode = newEncode end
end

return {merger = merger, decoder = decoder, encoder = encoder}
