...
function Socket:_onSocketFinish()
    ...
    if not self.readable then return self:destroy() end
end

function Socket:_onSocketEnd() self:once("end", function() self:destroy() end) end

function Socket:bind(ip, port) uv.tcp_bind(self._handle, ip, tonumber(port)) end

function Socket:address() return uv.tcp_getpeername(self._handle) end
...
function Socket:shutdown(callback)
    if self.destroyed == true and callback then return callback() end

    if uv.is_closing(self._handle) and callback then return callback() end

    uv.shutdown(self._handle, callback)
end

function Socket:nodelay(enable) uv.tcp_nodelay(self._handle, enable) end

function Socket:keepalive(enable, delay)
    uv.tcp_keepalive(self._handle, enable, delay)
end

function Socket:getSendBufferSize() return uv.send_buffer_size(self._handle) end

function Socket:getRecvBufferSize() return uv.recv_buffer_size(self._handle) end
...
function Socket:connect(...)
    ...
    if not options.host then options.host = "0.0.0.0" end

    timer.active(self)
    self._connecting = true

    if not self._handle then self._handle = uv.new_tcp() end

    local _, derr = uv.getaddrinfo(options.host, options.port,
                                   {socktype = "stream"}, function(err, res)
        timer.active(self)
        if err then return self:destroy(err) end
        if self.destroyed then return end
        local _, terr = uv.tcp_connect(self._handle, res[1].addr, res[1].port,
                                       function(err)
            if err then return self:destroy(err) end
            timer.active(self)
            self._connecting = false
        ...
        if terr ~= nil then self:destroy(terr) end
    end)
    if derr ~= nil then return self:destroy(derr) end
...
function Socket:destroy(exception, callback)
    callback = callback or function() end
    if self.destroyed == true or self._handle == nil then return callback() end
    ...
    if exception then
        timer.setImmediate(function() self:emit("error", exception) end)
    end
end
...
function Socket:getsockname() return uv.tcp_getsockname(self._handle) end
--
...
function Server:init(options, connectionListener)
    ...
    if options.handle then self._handle = options.handle end
end

function Server:destroy(err, callback) self._handle:destroy(err, callback) end
...
function Server:listen(port, ... --[[ ip, callback --]] )
    ...
    self._handle:listen()
    self._handle:on("connection",
                    function(client) self:emit("connection", client) end)

    if callback then timer.setImmediate(callback) end

    return self
end

function Server:address()
    if self._handle then return self._handle:getsockname() end
    return
end

function Server:close(callback) self:destroy(nil, callback) end
