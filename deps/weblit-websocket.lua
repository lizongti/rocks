...
local function websocketHandler(options, handler)
    return function(req, res, go)
        ...
        if not (req.method == "GET" and upgrade and
            upgrade:lower():find("websocket", 1, true) and connection and
            connection:lower():find("upgrade", 1, true)) then return go() end

        if options.filter and not options.filter(req) then return go() end
        ...
        if protocol then
            ...
            if not foundProtocol then return go() end
        end

        -- Make sure it's a new client speaking v13 of the protocol
        assert(tonumber(headers["sec-websocket-version"]) >= 13,
               "only websocket protocol v13 supported")

        -- Get the security key
        local key = assert(headers["sec-websocket-key"],
        ...
        if protocol then headers["Sec-WebSocket-Protocol"] = protocol end
        function res.upgrade(read, write, updateDecoder, updateEncoder)
            ...
            read, write = wrapIo(read, write,
                                 {mask = false, heartbeat = options.heartbeat})
            local success, err = pcall(handler, req, read, write)
            if not success then
                print(err)
                write({opcode = 1, payload = err})
                return write()
            end
        end
    end
...
function server.websocket(options, handler)
    server.route({method = "GET", path = options.path, host = options.host},
