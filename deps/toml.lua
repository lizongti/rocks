...
TOML.parse = function(toml, options)
    ...
    local function skipWhitespace() while (char():match(ws)) do step() end end
    ...
    local function split(str, delim)
        ...
        local result = {}
        local append = delim
        if delim:match("%%") then append = delim:gsub("%%", "") end
            ...
            if quoteType == '"' and char() == "\\" then
                if multiline and char(1):match(nl) then
                    ...
                    elseif char(1) == "u" then
                        ...
                        step(5)
                        uni = tonumber(uni, 16)
                        if (uni >= 0 and uni <= 0xd7ff) and
                            not (uni >= 0xe000 and uni <= 0x10ffff) then
                            str = str .. utf(uni)
                        else
    ...
    local function parseNumber()
        ...
        while (bounds()) do
            ...
            elseif char():match(ws) or char() == "#" or char():match(nl) or
            ...
            elseif char() == "T" or char() == "Z" then
                ...
                while (bounds()) do
                    if char() == "," or char() == "]" or char() == "#" or
                        char():match(nl) or char():match(ws) then
                        break
                    end
                    num = num .. char()
                    step()
    ...
    local function parseBoolean()
        ...
        skipWhitespace()
        if char() == "#" then while (not char():match(nl)) do step() end end
    ...
    while (cursor <= toml:len()) do
        ...
        elseif char() == "[" then
            ...
            local function processKey(isLast)
                ...
                if isLast and obj[buffer] and not tableArray and #obj[buffer] >
                    0 then err("Cannot redefine table", true) end
