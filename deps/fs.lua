...
function fs.close(fd, callback) return adapt(callback, uv.fs_close, fd) end
function fs.closeSync(fd) return uv.fs_close(fd) end
...
function fs.read(fd, size, offset, callback)
    ...
    if size == nil then size = 4096 end
...
function fs.unlink(path, callback) return adapt(callback, uv.fs_unlink, path) end
function fs.unlinkSync(path) return uv.fs_unlink(path) end
...
function fs.mkdir(path, mode, callback)
    local mt = type(mode)
    if (mt == 'function' or mt == 'thread') and (callback == nil) then
        callback, mode = mode, nil
    end
    if mode == nil then
        mode = 511 -- 0777
...
function fs.mkdirpSync(path, mode)
    ...
    if success or string.match(err, "^EEXIST") then return true end
...
function fs.mkdtempSync(template) return uv.fs_mkdtemp(template) end
function fs.rmdir(path, callback) return adapt(callback, uv.fs_rmdir, path) end
function fs.rmdirSync(path) return uv.fs_rmdir(path) end
...
function fs.readdir(path, callback) return adapt(callback, readdir, path) end
...
function fs.scandir(path, callback) return adapt(callback, scandir, path) end
...
function fs.stat(path, callback) return adapt(callback, uv.fs_stat, path) end
function fs.statSync(path) return uv.fs_stat(path) end
function fs.fstat(fd, callback) return adapt(callback, uv.fs_fstat, fd) end
function fs.fstatSync(fd) return uv.fs_fstat(fd) end
function fs.lstat(path, callback) return adapt(callback, uv.fs_lstat, path) end
function fs.lstatSync(path) return uv.fs_lstat(path) end
...
function fs.fsync(fd, callback) return adapt(callback, uv.fs_fsync, fd) end
function fs.fsyncSync(fd) return uv.fs_fsync(fd) end
function fs.fdatasync(fd, callback) return adapt(callback, uv.fs_fdatasync, fd) end
function fs.fdatasyncSync(fd) return uv.fs_fdatasync(fd) end
...
function fs.access(path, flags, callback)
    local ft = type(flags)
    if (ft == 'function' or ft == 'thread') and (callback == nil) then
        callback, flags = flags, nil
    end
...
function fs.chmodSync(path, mode) return uv.fs_chmod(path, mode) end
function fs.fchmod(fd, mode, callback)
    return adapt(callback, uv.fs_fchmod, fd, mode)
end
function fs.fchmodSync(fd, mode) return uv.fs_fchmod(fd, mode) end
function fs.utime(path, atime, mtime, callback)
    return adapt(callback, uv.fs_utime, path, atime, mtime)
end
function fs.utimeSync(path, atime, mtime) return uv.fs_utime(path, atime, mtime) end
function fs.futime(fd, atime, mtime, callback)
    return adapt(callback, uv.fs_futime, fd, atime, mtime)
end
function fs.futimeSync(fd, atime, mtime) return uv.fs_futime(fd, atime, mtime) end
function fs.link(path, newPath, callback)
    return adapt(callback, uv.fs_link, path, newPath)
end
function fs.linkSync(path, newPath) return uv.fs_link(path, newPath) end
...
function fs.readlink(path, callback) return
    adapt(callback, uv.fs_readlink, path) end
function fs.readlinkSync(path) return uv.fs_readlink(path) end
function fs.chown(path, uid, gid, callback)
    return adapt(callback, uv.fs_chown, path, uid, gid)
end
function fs.chownSync(path, uid, gid) return uv.fs_chown(path, uid, gid) end
function fs.fchown(fd, uid, gid, callback)
    return adapt(callback, uv.fs_fchown, fd, uid, gid)
end
function fs.fchownSync(fd, uid, gid) return uv.fs_fchown(fd, uid, gid) end
...
function fs.readFile(path, callback) return adapt(callback, readFile, path) end
function fs.readFileSync(path)
    ...
    if stat then
        if stat.size > 0 then
            ...
            if not err then chunk = table.concat(chunks) end
        end
    end
    uv.fs_close(fd, noop)
    return chunk, err
...
function fs.WriteStream:close() self:destroy() end
...
function fs.ReadStream:_read(n)
    if not self.fd then return self:once('open', bind(self._read, self, n)) end
...
function fs.ReadStream:close() self:destroy() end
...
function fs.appendFile(filename, data, callback)
    ...
    local function write(fd, offset, buffer, callback)
        local function onWrite(err, written)
            if err then
                return fs.close(fd, function() callback(err) end)
            end
