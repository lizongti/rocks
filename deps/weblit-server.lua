...
function headerMeta:__index(name)
    if type(name) ~= "string" then return rawget(self, name) end
    name = name:lower()
    for i = 1, #self do
...
function headerMeta:__newindex(name, value)
    ...
    if type(name) ~= "string" then return rawset(self, name, value) end
    -- First remove any existing pairs with matching key
    local lowerName = name:lower()
    for i = #self, 1, -1 do
        if self[i][1]:lower() == lowerName then table.remove(self, i) end
    end
...
local function newServer(run)
    ...
    local function handleRequest(head, input, socket)
        ...
        for i = 1, #head do req.headers[i] = head[i] end
        ...
        local out = {code = res.code, keepAlive = res.keepAlive}
        for i = 1, #res.headers do out[i] = res.headers[i] end
        return out, res.body, res.upgrade
    end

    local function handleConnection(read, write, socket, updateDecoder,
                                    updateEncoder)
        ...
        for head in read do
            ...
            local res, body, upgrade = handleRequest(head, #parts > 0 and
                                                         table.concat(parts) or
                                                         nil, socket)
            write(res)
            if upgrade then
                return
                    upgrade(read, write, updateDecoder, updateEncoder, socket)
            end
            write(body)
            if not (res.keepAlive and head.keepAlive) then break end
        end
        write()

    end

    function server.setRun(newRun) run = newRun end

    function server.bind(options)
        if not options.port then
            local getuid = require('luv').getuid
    ...
    function server.start()
        if #bindings == 0 then server.bind({}) end
        ...
        for i = 1, #bindings do
            ...
            if options.host then show(options) end
        end
...
return {newServer = newServer, headerMeta = headerMeta}
