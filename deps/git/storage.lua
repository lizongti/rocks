...
storage.leaves(path) -> iter - Iterate over node children of path
    ...
    function storage.put(path, data)
        ...
        success, err = fs.write(fd, data)
        if success then success, err = fs.fchmod(fd, 256) end
        fs.close(fd)
        assert(success, err)
    end
    ...
    local function iter(path, filter)
        ...
        if not it then
            if err:match("^ENOENT:") then return function() end end
            assert(it, err)
        end
        return function()
            while true do
    ...
    function storage.nodes(path) return iter(path, "directory") end

    function storage.leaves(path) return iter(path, "file") end
