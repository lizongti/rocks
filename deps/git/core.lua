...
local function binToHex(c) return string.format("%02x", string.byte(c, 1)) end
...
function modes.toType(mode)
    -- 0160000
    return mode == 57344 and "commit" -- 040000
    or mode == 16384 and "tree" -- (mode & 0140000) == 0100000
    or bit.band(mode, 49152) == 32768 and "blob" or "unknown"
end
...
local function parsePerson(raw)
    ...
    local name, email, seconds, sign, hours, minutes =
        string.match(raw, pattern)
    local offset = tonumber(hours) * 60 + tonumber(minutes)
    if sign == "-" then offset = -offset end
    return {
        name = name,
        email = email,
        date = {seconds = tonumber(seconds), offset = offset}
    }
end
...
function decoders.blob(raw) return raw end
...
function encoders.tree(tree)
    ...
    for i = 1, #parts do
        local entry = parts[i]
        parts[i] = string.format("%o %s\0", entry.mode, entry.name) ..
...
function decoders.tree(raw)
    ...
    while start <= length do
        ...
        tree[#tree + 1] = {name = name, mode = mode, hash = hash}
        start = e + 1
    end
    return tree
end
...
function encoders.tag(tag)
    ...
    return string.format("object %s\ntype %s\ntag %s\ntagger %s\n\n%s",
                         tag.object, tag.type, tag.tag,
                         formatPerson(tag.tagger), tag.message)
end
...
function decoders.tag(raw)
    ...
    while true do
        ...
        if name == "tagger" then value = parsePerson(value) end
        data[name] = value
        start = e + 1
    end
    return data
...
function encoders.commit(commit)
    ...
    return string.format("tree %s\n%sauthor %s\ncommitter %s\n\n%s",
                         commit.tree, table.concat(parents),
                         formatPerson(commit.author),
