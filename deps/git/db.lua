...
local function readUint32(buffer, offset)
    ...
    return bor(lshift(byte(buffer, offset + 1), 24),
               lshift(byte(buffer, offset + 2), 16),
               lshift(byte(buffer, offset + 3), 8), byte(buffer, offset + 4))
end

local function readUint64(buffer, offset)
    offset = offset or 0
    assert(#buffer >= offset + 8, "not enough buffer")
    return (lshift(byte(buffer, offset + 1), 24) +
               lshift(byte(buffer, offset + 2), 16) +
               lshift(byte(buffer, offset + 3), 8) + byte(buffer, offset + 4)) *
               0x100000000 + lshift(byte(buffer, offset + 5), 24) +
               lshift(byte(buffer, offset + 6), 16) +
               lshift(byte(buffer, offset + 7), 8) + byte(buffer, offset + 8)
end
...
[gc]
    ...
    local function makePack(packHash)
        ...
        local function timeout() coroutine.wrap(close)() end
        ...
        assert(fs.read(indexFd, 8, 0) == '\255tOc\0\0\0\2',
               'Only pack index v2 supported')
        ...
        local function loadHash(hash) -- > offset
            ...
            local first = prefix == 0 and 0 or
                              readUint32(
                                  assert(
                                      fs.read(indexFd, 4, 8 + (prefix - 1) * 4)))
            ...
            for index = first, last do
                ...
                if foundHash == hash then
                    return offsets[index + 1], lengths[index + 1]
                end
            end
        end
        ...
        local function loadRaw(offset, length) -- >raw
            ...
            assert(#raw == size,
                   "inflate error or size mismatch at offset " .. offset)
        ...
        function pack.load(hash) -- > raw
            if not pack then return makePack(packHash).load(hash) end
            timer:again()
        ...
        packs[packHash] = pack
        pack.waiting = nil
        for i = 1, #waiting do assert(coroutine.resume(waiting[i], pack)) end
    ...
    function db.saveAs(kind, value)
        if type(value) ~= "string" then value = encoders[kind](value) end
    ...
    function db.resolve(ref)
        ...
        return db.getRef(ref) or db.getRef("refs/heads/" .. ref) or
                   db.getRef("refs/tags/" .. ref)
    end
    ...
    local function makePackedIter(prefix, inner)
        ...
        if prefix:byte(-1) ~= 47 then prefix = prefix .. "/" end
        if inner then
