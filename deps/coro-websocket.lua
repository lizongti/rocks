...
local function parseUrl(url)
    local protocol, host, port, pathname =
        string.match(url, "^(wss?)://([^:/]+):?(%d*)(/?[^#?]*)")
    local tls
    if protocol == "ws" then
    ...
    return {host = host, port = port, tls = tls, pathname = pathname}
end
...
local function wrapIo(rawRead, rawWrite, options)
    ...
    local function read()
        while true do
            ...
            if not message then return cleanup() end
            if message.opcode < 8 then return message end
            if not closeSent then
                if message.opcode == 8 then
                    write {opcode = 8, payload = message.payload}
                elseif message.opcode == 9 then
                    write {opcode = 10, payload = message.payload}
                end
                return message
            end
        end
    ...
    if options.heartbeat then
        ...
        timer:start(interval, interval, function()
            coroutine.wrap(function()
                local success, err = write {opcode = 10, payload = ""}
                if not success then
                    timer:close()
                    print(err)
                end
...
local function connect(options)
    ...
    if not read then return nil, write end
    ...
    local success, err = websocketCodec.handshake(
            ...
            if headers then
                for i = 1, #headers do req[#req + 1] = headers[i] end
            end
            write(req)
            res = read()
    ...
    read, write = wrapIo(read, write,
                         {mask = true, heartbeat = options.heartbeat})
...
return {parseUrl = parseUrl, wrapIo = wrapIo, connect = connect}
