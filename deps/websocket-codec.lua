...
local function applyMask(data, mask)
    ...
    local out = {}
    for i = 1, #data do
        out[i] = char(bxor(byte(data, i), bytes[(i - 1) % 4]))
    end
    return concat(out)
end
...
local function decode(chunk, index)
    ...
    if len == 126 then
        ...
        len = bor(lshift(byte(chunk, start + 3), 8), byte(chunk, start + 4))
        offset = 4
    elseif len == 127 then
        if length < 10 then return end
        len = bor(lshift(byte(chunk, start + 3), 24),
                  lshift(byte(chunk, start + 4), 16),
                  lshift(byte(chunk, start + 5), 8), byte(chunk, start + 6)) *
                  0x100000000 +
                  bor(lshift(byte(chunk, start + 7), 24),
                      lshift(byte(chunk, start + 8), 16),
                      lshift(byte(chunk, start + 9), 8), byte(chunk, start + 10))
        offset = 10
    else
        offset = 2
    end
    ...
    if mask then payload = applyMask(payload, sub(chunk, offset - 3, offset)) end
    return {
...
local function encode(item)
    if type(item) == "string" then item = {opcode = 2, payload = item} end
    ...
    local chars = {
        char(bor(fin and 0x80 or 0, rsv1 and 0x40 or 0, rsv2 and 0x20 or 0,
                 rsv3 and 0x10 or 0, opcode)), char(
            bor(mask and 0x80 or 0,
...
local function hexToBin(cc) return string.char(tonumber(cc, 16)) end
...
local function handleHandshake(head, protocol)
    ...
    if not key then return nil, "websocket security key missing" end
    ...
    if protocol then res[#res + 1] = {"Sec-WebSocket-Protocol", protocol} end
