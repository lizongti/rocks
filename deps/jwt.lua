...
local function normalizeAlgorithm(options)
    ...
    assert(type(algorithm) == "string", "Algorithm must be a string")
...
local function sign(payload, options)
    ...
    local header = options.header or {typ = "JWT", alg = algorithm}
    ...
    if not options.noTimestamp then data.iat = now end
    ...
    local message = base64Encode(jsonStringify(header)) .. "." ..
...
local function verify(token, options)
    local algorithm = normalizeAlgorithm(options)
    local message, signature = assert(string.match(token,
                                                   "^([^.]+%.[^.]+)%.([^.]*)$"))
    signature = base64Decode(signature)
    if options.publicKey then error "TODO: verify signature using public key" end
    local expected = calculateSignature(message, algorithm, options)
    if expected ~= signature then return nil, "Signature validation failure" end
