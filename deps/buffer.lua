...
function Buffer:initialize(length)
    if type(length) == "number" then
        self.length = length
        self.ctype =
            ffi.gc(ffi.cast("unsigned char*", C.malloc(length)), C.free)
    elseif type(length) == "string" then
        local string = length
        self.length = #string
        self.ctype = ffi.gc(ffi.cast("unsigned char*", C.malloc(self.length)),
                            C.free)
...
function Buffer.meta:__tostring() return ffi.string(self.ctype, self.length) end

function Buffer.meta:__concat(other) return tostring(self) .. tostring(other) end
...
function Buffer:inspect()
    local parts = {}
    for i = 1, self.length do parts[i] = bit.tohex(self[i], 2) end
    return "<Buffer " .. table.concat(parts, " ") .. ">"
end

local function complement8(value) return value < 0x80 and value or value - 0x100 end

function Buffer:readUInt8(offset) return self[offset] end

function Buffer:readInt8(offset) return complement8(self[offset]) end
...
function Buffer:readUInt32LE(offset)
    return self[offset + 3] * 0x1000000 + bit.lshift(self[offset + 2], 16) +
               bit.lshift(self[offset + 1], 8) + self[offset]
end

function Buffer:readUInt32BE(offset)
    return self[offset] * 0x1000000 + bit.lshift(self[offset + 1], 16) +
               bit.lshift(self[offset + 2], 8) + self[offset + 3]
end

function Buffer:readInt32LE(offset) return bit.tobit(self:readUInt32LE(offset)) end

function Buffer:readInt32BE(offset) return bit.tobit(self:readUInt32BE(offset)) end

function Buffer:writeUInt8(offset, value) self[offset] = value end

function Buffer:writeInt8(offset, value) return self:writeUInt8(offset, value) end
...
function Buffer.isBuffer(b) return instanceof(b, Buffer) end
