...
local function isTruthyString(obj) return obj and obj ~= "" end
...
local function parse(url, parseQueryString)
    ...
    if url ~= '' then
        ...
        local temp
        temp = url:match("^[^?]*")
        if temp ~= '' then pathname = temp end
        temp = url:sub((pathname and #pathname or 0) + 1)
        if temp ~= '' then search = temp end
        if search then
            temp = search:sub(2)
            if temp ~= '' then query = temp end
        end
    end

    if parseQueryString then query = querystring.parse(query) end
...
local function resolveObject(source, relative)
    ...
    local isSourceAbs = isTruthyString(result.pathname) and
                            string.sub(result.pathname, 1, 1) == '/'
    local isRelAbs = isTruthyString(relative.host) or
                         isTruthyString(relative.pathname) and
                         string.sub(relative.pathname, 1, 1) == '/'
    local mustEndAbs = isRelAbs or isSourceAbs or
                           (isTruthyString(result.host) and
                               isTruthyString(relative.pathname))
    local removeAllDots = mustEndAbs
    local srcPath = (isTruthyString(result.pathname) and
                        splitString(result.pathname, '/')) or {}
    local relPath = (isTruthyString(relative.pathname) and
                        splitString(relative.pathname, '/')) or {}
    ...
    elseif #relPath > 0 then
        ...
        for _, v in ipairs(relPath) do table.insert(srcPath, v) end
        result.search = relative.search
        result.query = relative.query
    ...
    local hasTrailingSlash = (isTruthyString(result.host) or
                                 isTruthyString(relative.host) or #srcPath > 1) and
    ...
    if mustEndAbs and srcPath[1] ~= "" and
        (not srcPath[1] or string.sub(srcPath[1], 1, 1) ~= "/") then
        table.insert(srcPath, 1, "")
    end
    ...
    local isAbsolute = srcPath[1] == "" or
                           (srcPath[1] and string.sub(srcPath[1], 1, 1) == "/")
    mustEndAbs = mustEndAbs or (isTruthyString(result.host) and #srcPath > 0)

    if mustEndAbs and not isAbsolute then table.insert(srcPath, 1, "") end
