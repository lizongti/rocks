...
local quote, quote2, dquote, dquote2, obracket, cbracket, obrace, cbrace, comma,
      equals, controls
...
function color(colorName) return '\27[' .. (theme[colorName] or '0') .. 'm' end
...
local function stringEscape(c) return controls[string.byte(c, 1)] end
...
function dump(value, recurse, nocolor)
    ...
    local function write(text, length)
        ...
        if offset > width then return dump(stack) end
    end
    ...
    local function unindent() stack[#stack] = nil end
...
function _G.print(...)
    local n = select('#', ...)
    local arguments = {...}
    for i = 1, n do arguments[i] = tostring(arguments[i]) end
...
function prettyPrint(...)
    local n = select('#', ...)
    local arguments = {...}
    for i = 1, n do arguments[i] = dump(arguments[i]) end
    console_write(stdout, table.concat(arguments, "\t"))
end
