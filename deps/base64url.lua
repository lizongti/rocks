...
local function base64Encode(str)
    ...
    for i = 1, #str, 3 do
        ...
        parts[j] = char( -- Higher 6 bits of a
        byte(codes, rshift(a, 2) + 1), -- Lower 2 bits of a + high 4 bits of b
        byte(codes, bor(lshift(band(a, 3), 4), b and rshift(b, 4) or 0) + 1),
        -- Low 4 bits of b + High 2 bits of c
        b and
            byte(codes, bor(lshift(band(b, 15), 2), c and rshift(c, 6) or 0) + 1) or
            61, -- 61 is '='
        -- Lower 6 bits of c
    ...
    if #parts > 0 then
        ...
        if i then parts[j] = string.sub(last, 1, i - 1) end
    end
    return concat(parts)
end
